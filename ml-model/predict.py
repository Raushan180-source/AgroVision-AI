...
DISEASE_INFO = {
    ...
    'Potato___Late_blight': {
        ...
        'description': 'Caused by Phytophthora infestans — the same pathogen that caused the Irish Famine.',
        'recommendations': {
...
def get_disease_info(class_name: str) -> dict:
    """Get disease metadata by class name."""
    return DISEASE_INFO.get(class_name, {**DEFAULT_INFO, 'display': class_name.replace('_', ' ')})


def load_class_indices(path='class_indices.json') -> dict:
    """Load class index mapping."""
    try:
        with open(path) as f:
            indices = json.load(f)
        return {v: k for k, v in indices.items()}  # invert: index -> class_name
